...
void main() {
  ...
  test('initial state should be empty', () {
    expect(tvShowDetailBloc.state, TVShowEmptyState());
  });

  blocTest<TVShowDetailBloc, TVShowState>(
    'Should emit [Loading, Error] when get detail is unsuccessful',
    build: () {
      when(mockGetTVShowDetail.execute(tTVDetail.id))
          .thenAnswer((_) async => Left(ServerFailure('Server Failure')));
      return tvShowDetailBloc;
    },
    act: (bloc) => bloc.add(TVShowDetailEvent(tvShowId: tTVDetail.id)),
    expect: () => [
      TVShowLoadingState(),
      TVShowErrorState(message: 'Server Failure'),
    ],
    verify: (bloc) {
      verify(mockGetTVShowDetail.execute(tTVDetail.id));
    },
  );
  ...
  blocTest<TVShowDetailBloc, TVShowState>(
    ...
    expect: () => [
      TVShowLoadingState(),
      TVShowHasDataState(result: tTVDetail),
    ],
    verify: (bloc) {
      verify(mockGetTVShowDetail.execute(tTVDetail.id));
    },
